...
function upload_files() {
    ...
    ////////////////////////////////////////////////////////////////////////////
    $upfile = $_SERVER['DOCUMENT_ROOT'].'/2ndoDAW/NiponTour/media/'.$_FILES['avatar']['name'];
    if (is_uploaded_file($_FILES['file']['tmp_name'])){
        if (is_file($_FILES['file']['tmp_name'])) {
            ...
            $upfile = $_SERVER['DOCUMENT_ROOT'].'/2ndoDAW/NiponTour/media/'.$nombreFichero;
        }else{
                $error .=   "Invalid File...";
        }
    } 
...
function remove_files(){
	$name = $_POST["filename"];
	if(file_exists($_SERVER['DOCUMENT_ROOT'].'/2ndoDAW/NiponTour/media/'.$name)){
		unlink($_SERVER['DOCUMENT_ROOT'].'/2ndoDAW/NiponTour/media/'.$name);
		return true;
	}else{
		return false;
	}
